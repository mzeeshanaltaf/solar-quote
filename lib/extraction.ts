...
- For the location: "rawAddress" is the FULL service/supply address exactly as printed (including house/flat number and street) — only ever transcribe rawAddress, never infer or complete it. Separately, break out COARSE components: "addressTown" (town/area/locality), "addressCity" (city), "addressState" (state/province/region), and "addressCountry" (country) — these components must NOT contain the house/flat number or street.
- The coarse components are the one exception to "never guess": when a component is NOT explicitly printed, INFER it. Electricity utilities are very often named after the city or region they serve — use the utility company name as your primary clue, then the language and currency. Examples: "Lahore Electric Supply Company (LESCO)" → city Lahore, state Punjab, country Pakistan; "K-Electric" → Karachi, Sindh, Pakistan; "Florida Power & Light" → state Florida, country United States; "Tokyo Electric Power" → Tokyo, Japan. Once you've inferred the city/region this way, infer the state and country that contain it. Mark every inferred component as "low" confidence. If you genuinely cannot tell, return null for that component (still never guess rawAddress).
